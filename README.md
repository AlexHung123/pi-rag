...
STT_MOCK=false
STT_BASE_URL=http://192.168.1.61:8002
...
**Video:** Prefer a unified remote STT (e.g. `http://192.168.1.61:8002`) that accepts `.mp4` and runs ffmpeg in-process. pi-rag sends the original file; set `STT_CLIENT_TRANSCODE=true` only if the STT server cannot handle video.
